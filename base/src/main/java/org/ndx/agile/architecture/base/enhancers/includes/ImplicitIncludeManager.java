...
/**
 * Enhancer allowing includes to be auto-generated for each element having
 * interesting content.
 * 
 * Let's take an example. Suppose you want to add some documentation for a
 * component code. Then you just have to write the documentation in the file
 * ${asciidoc.source.dir}/07-code/${component name}.adoc and it will be
 * automagically included in documentation.
 * 
...
public class ImplicitIncludeManager extends ModelElementAdapter {
	...
	public void setDocumentsFolder(File sourceDir) {
		if (sourceDir == null) {
		...
		if (potentialFile.exists()) {
			...
			if (!targetFile.exists()) {
				try {
					...
					FileUtils.write(targetFile, String.format("include::%s[leveloffset=+1]", relativePath.toString()),
							"UTF-8");
				} catch (IOException e) {
